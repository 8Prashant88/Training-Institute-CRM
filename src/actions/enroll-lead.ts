...
const enrollLeadSchema = z.object({
  ...
  batchId: z.uuid({
    error: "Select a valid batch.",
  }),
});

export type EnrollLeadActionInput = z.infer<
  typeof enrollLeadSchema
>;

type EnrollLeadFieldErrors = Partial<
  Record<keyof EnrollLeadActionInput, string>
>;
...
export type EnrollLeadActionResult =
  ...
  | {
      ...
      message: string;
      data?: undefined;
      fieldErrors: EnrollLeadFieldErrors;
    };
  ...
  for (
    ...
    try {
      ...
      const isTransactionConflict =
        getDatabaseErrorCode(error) === "P2034";
...
export async function submitEnrollment(
  ...
  const result = enrollLeadSchema.safeParse(input);
